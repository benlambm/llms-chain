...
def _find_section_boundary(html_content: str, heading_text: str) -> Optional[int]:
    """Return the start index of a heading, if present."""
    pattern = rf"(?is)<h[1-6][^>]*>\s*{re.escape(heading_text)}\s*</h[1-6]>"
    match = re.search(pattern, html_content)
    return match.start() if match else None


def _is_within_tag(html_content: str, tag: str, pos: int) -> bool:
    """Check if a position is inside a simple tag scope."""
    before = html_content[:pos].lower()
...
def stage_d_inject_images(state: PipelineState) -> bool:
    ...
    summary_start = _find_section_boundary(html_content, "Summary")
    glossary_start = _find_section_boundary(html_content, "Glossary of Key Terms")
    content_end_candidates = [pos for pos in [summary_start, glossary_start] if pos is not None]
    content_end_idx = min(content_end_candidates) if content_end_candidates else None
    skipped_outside_core = []
    skipped_wrapped = []
    placements = []
    ...
    for plan in state.image_plans:
        ...
        if content_end_idx is not None and match.start() >= content_end_idx:
            skipped_outside_core.append((match, plan, img_src))
            continue
        if _is_within_tag(html_content, "details", match.start()) or _is_within_tag(html_content, "dl", match.start()):
            skipped_wrapped.append((match, plan, img_src))
            continue

        placements.append((match, plan, img_src))

    if not placements and skipped_outside_core:
        placements = skipped_outside_core
    if not placements and skipped_wrapped:
        placements = skipped_wrapped

    ordered = sorted(placements, key=lambda item: item[0].start())
    insertions = []
    for display_number, (match, plan, img_src) in enumerate(ordered, start=1):
...
<figure style="margin: 2em 0; max-width: 75ch;">
  ...
  <figcaption style="padding: 0.75em 1em; font-style: italic; background-color: #F5F5F5; text-align: center; font-family: system-ui, -apple-system, 'Segoe UI', sans-serif; font-size: 16px; color: #333333; border-bottom-left-radius: 4px; border-bottom-right-radius: 4px;">
    {caption_full} <em>(Generated by AI)</em>
  </figcaption>
</figure>
"""
        insertions.append((match.end(), figure_html, display_number))

    for end_pos, figure_html, display_number in sorted(insertions, key=lambda item: item[0], reverse=True):
